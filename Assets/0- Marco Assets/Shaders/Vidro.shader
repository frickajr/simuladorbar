    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float3 _RefractionTexture_var = UnpackNormal(tex2D(_RefractionTexture,TRANSFORM_TEX(node_27, _RefractionTexture)));
                float2 sceneUVs = float2(1,grabSign)*i.screenPos.xy*0.5+0.5 + (_RefractionTexture_var.rgb.rg*(_RefractionIntensity*0.2));
                float4 sceneColor = tex2D(_GrabTexture, sceneUVs);
                float3x3 tangentTransform = float3x3( i.tangentDir, i.bitangentDir, i.normalDir);
/////// Vectors:
                float3 viewDirection = normalize(_WorldSpaceCameraPos.xyz - i.posWorld.xyz);
                float3 normalLocal = lerp(float3(0,0,1),_RefractionTexture_var.rgb,_RefractionIntensity);
...
////// Specular:
                ...
                float3 diffuseColor = _CordoVidro.rgb; // Need this for specular when using metallic
